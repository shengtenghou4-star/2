...
export function HourCandidateExplorer({ input, onUse }: { input: BirthInput; onUse: (input: BirthInput) => void }) {
  const result = useMemo(() => {
    try {
      return {
        candidates: HOURS.map((hour, index) => buildCandidate(input, hour, BRANCH_NAMES[index])),
        error: '',
      };
    } catch (reason) {
      return {
        candidates: [] as Candidate[],
        error: reason instanceof Error ? reason.message : '当前输入无法生成时辰候选。',
      };
    }
  }, [input]);
  ...
  if (result.error) {
    return (
      <section className="hour-candidate-explorer panel">
        <div className="section-heading compact"><span>07</span><div><h2>时辰候选校验</h2><p>比较十二时辰下哪些结论稳定，哪些高度依赖时柱</p></div></div>
        <p className="error">候选暂不可用：{result.error} 请完成有效日期输入并重新生成命盘。</p>
      </section>
    );
  }
  ...
  ].map((item) => ({ ...item, values: frequency(result.candidates, item.key) }));
